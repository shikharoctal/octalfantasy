...
class ViewPagerTabView: UIView {
    ...
    fileprivate func setupBasicTab(condition:SetupCondition, options:ViewPagerOptions, tab:ViewPagerTab) {
        ...
        case .fitAllTabs:
            ...
            titleLabel?.frame = CGRect(x: 10, y: 5, width: self.frame.size.width-20, height: self.frame.size.height-10)
        ...
        case .distributeNormally:
            ...
            let newFrame = CGRect(x: currentFrame.origin.x, y: currentFrame.origin.y + 5, width: labelWidth-20, height: currentFrame.height-10)
            self.frame = newFrame
            
            // Setting TitleLabel frame
            titleLabel?.frame = CGRect(x: 10, y: 0, width: labelWidth, height: currentFrame.height)
        }
