...
class ViewPagerController:UIViewController {
    ...
    fileprivate func setupTabContainerView() {
        ...
        self.view.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-(tabContainerYPosition)-[v0(tabViewHeight)]", options: NSLayoutConstraint.FormatOptions(), metrics: metrics, views: viewDict))
    }
    
    func setCurrentTabSelected(){
        ...
        for (index,eachTab) in tabsList.enumerated() {
            ...
            tabView.tag = index
            tabView.titleLabel!.cornerRadius = tabView.titleLabel!.frame.size.height / 2
            tabView.titleLabel?.clipsToBounds = true
            tabView.titleLabel?.textColor = ((index == currentPageIndex) ? .white : .black)
            tabView.titleLabel?.backgroundColor = ((index == currentPageIndex) ? UIColor.appThemeColor : .white)
...
//                tabView.titleLabel!.cornerRadius = tabView.titleLabel!.frame.size.height / 2
//                tabView.titleLabel?.clipsToBounds = true
//                tabView.titleLabel?.backgroundColor = UIColor.white
//                tabsViewList.append(tabView)
//                tabContainer.addSubview(tabView)
//            }
